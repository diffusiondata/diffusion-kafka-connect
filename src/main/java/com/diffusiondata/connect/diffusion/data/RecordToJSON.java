...
/**
 * Parses a SinkRecord that contains a value schema into a Diffusion-compatible
 * JSON instance.
 */
public final class RecordToJSON {
    private static final JSONDataType JSON_TYPE =
        Diffusion.dataTypes().json();
    private static final CBORFactory CBOR = new CBORFactory();

    /**
     * Parse a {@link SinkRecord} instance and return a Diffusion-compatible
     * {@link JSON} value.
     * <p>
     * If the provided record contains a schema, then the value will be parsed
     * according to the associated schema. If the schema is null, then the
     * parser will attempt to determine the appropriate serialisation type based
     * on the provided value's class.
     * <p>
     * If the record contains a value of an unknown type, or is unable to be
     * serialised to JSON (e.g. containing a Map that has non-primitive keys),
     * it will throw a {@link DataException}.
     *
        ...
        if (value == null) {
            if (schema.isOptional()) {
                ...
                throw new DataException(
                    "Encountered null value for required schema: " + schema);
            }
        }
        ...
        switch (type) {
            ...
            case MAP:
                if (schema.keySchema() != null &&
                    schema.keySchema().type() != Type.STRING) {

                    throw new DataException(
                        "Unable to serialise non-primitive keyed map values");
                }
                ...
                for (Entry<?, ?> entry : map.entrySet()) {
                    generator.writeFieldName(
                        convertToString(schema.keySchema(), entry.getKey()));
                    writeValue(entry.getValue(), schema.valueSchema(),
                        generator);
                }
            ...
            default:
                throw new DataException(
                    "Encountered unsupported value schema type: " + schema);
        }
    }
    ...
    private static void writeValue(
        Object value, CBORGenerator generator, boolean isKey) throws IOException {
        if (value == null) {
            generator.writeNull();
        }
            ...
            throw new DataException(
                "Encountered unsupported value type: " + value.getClass());
        }
    }
}
