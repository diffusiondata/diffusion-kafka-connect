 ...
 *******************************************************************************/
package com.diffusiondata.connect.diffusion.client;
 ...
 * <p>
 * Abstracts common operations to more naturally fit behavior required for the
 * Connect framework.
 */
public interface DiffusionClient {
     ...
     * @return A completable future representing the state of the subscribe
     *     operation
     */
    CompletableFuture<?> subscribe(
        String topicSelector, SubscriptionStream stream);

    /**
     * Update a topic on Diffusion.
     * <p>
     * If the topic does not exist, it will be created and then the update will
     * be retried.
     * <p>
     * The returned future will contain either the sucesfully published value,
     * or an ExecutionException indicating the point of failure.
     *
    ...
    interface SubscriptionStream {
         ...
         */
        void onMessage(
            String topicPath, TopicSpecification specification, JSON value);
    }
}