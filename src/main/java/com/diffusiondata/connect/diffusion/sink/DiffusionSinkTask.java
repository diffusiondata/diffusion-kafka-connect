...
import com.diffusiondata.connect.diffusion.client.impl.DiffusionClientFactoryImpl;
...
import com.diffusiondata.connect.diffusion.data.RecordToJSON;
...
public class DiffusionSinkTask extends SinkTask implements Listener {
    private final Map<String, Map<Integer, List<CompletableFuture<JSON>>>> pendingOffsets =
        new HashMap<>();
        ...
        if (running) {
            for (SinkRecord record : records) {
                ...
                final String topicPath =
                    pathFromRecord(config.diffusionDestination(), record);
    ...
    private void markPending(
        SinkRecord record, CompletableFuture<JSON> result) {

        Map<Integer, List<CompletableFuture<JSON>>> pendingForTopic =
            pendingOffsets.get(record.topic());
        ...
        List<CompletableFuture<JSON>> pendingForPartition =
            pendingForTopic.get(record.kafkaPartition());
    ...
    public void flush(Map<TopicPartition, OffsetAndMetadata> partitionOffsets) {
        for (Entry<TopicPartition, OffsetAndMetadata> entry :
            ...
            Map<Integer, List<CompletableFuture<JSON>>> pendingForTopic =
                pendingOffsets.get(entry.getKey().topic());
            ...
            List<CompletableFuture<JSON>> pendingForPartition =
                pendingForTopic.get(entry.getKey().partition());
            ...
            try {
                ...
                throw new ConnectException(
                    "Unable to flush pending offsets", e);
            }
