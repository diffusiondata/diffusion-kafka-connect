 ...
 *******************************************************************************/
package com.pushtechnology.connect.diffusion.config;
...
public class DiffusionConfig extends AbstractConfig {
	...
	public static final String CONNECTION_GROUP = "Diffusion Connection Details";
	public static final String SETTINGS_GROUP = "Diffusion Topic Settings";
	
	private static ConfigDef base() {
		return new ConfigDef()
			.define(HOST, STRING, HIGH, "The hostname of the Diffusion instance to connect to", CONNECTION_GROUP, 0, LONG, "Diffusion Hostname")
			.define(PORT, INT, HIGH, "The port of the Diffusion instance to connect to", CONNECTION_GROUP, 1, MEDIUM, "Diffusion Port")
			.define(USERNAME, STRING, HIGH, "The username to connect to Diffusion with", CONNECTION_GROUP, 2, MEDIUM, "Username")
			.define(PASSWORD, Type.PASSWORD, HIGH, "The password to connect to Diffusion with", CONNECTION_GROUP, 3, LONG, "Password");
	}
	
	public static final ConfigDef SOURCE_CONFIG = base()
			.define(DIFFUSION_SELECTOR, STRING, HIGH, "Diffusion source topic selector", SETTINGS_GROUP, 0, LONG, "Diffusion topic selector")
			.define(KAFKA_TOPIC, STRING, HIGH, "The kafka topic pattern to publish data to", SETTINGS_GROUP, 1, LONG, "Kafka topic pattern")
			.define(POLL_INTERVAL, INT, POLL_INTERVAL_DEFAULT, LOW, "Time in milliseconds to wait for polling messages", SETTINGS_GROUP, 2, SHORT, "Poll interval")
			.define(POLL_BATCH_SIZE, INT, POLL_BATCH_SIZE_DEFAULT, LOW, "Number of messages to batch in each poll", SETTINGS_GROUP, 3, SHORT, "Poll batch size");
	
	public static final ConfigDef SINK_CONFIG = base()
			.define(DIFFUSION_DESTINATION, STRING, HIGH, "Diffusion destination topic pattern", SETTINGS_GROUP, 0, LONG, "Destination topic pattern");
