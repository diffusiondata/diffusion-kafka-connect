 ...
 *******************************************************************************/
package com.pushtechnology.connect.diffusion.util;

import org.apache.kafka.connect.data.Schema;
import org.apache.kafka.connect.sink.SinkRecord;

/**
 * Utility methods for constructing appropriate topic paths / keys from source objects, utilising format
 * patterns.
 * <P>
 * Patterns may be a simple string, or they may include one or more token patterns. These tokens will be
 * substituted with associated components of the object that the pattern is evaluated against.
 * <P>
...
public class TopicPathMapping {
	...
	public static String pathFromRecord(String pattern, SinkRecord record) {
		...
		if (keySchema != null) {
			if (keySchema.version() != null) {
				topic = topic.replace(KEY_VERSION, keySchema.version().toString());
			}
		}
		...
		if (valSchema != null) {
			if (valSchema.version() != null) {
				topic = topic.replace(VALUE_VERSION, valSchema.version().toString());
			}
		}
	...
	public static String pathFromTopic(String pattern, String topicPath) {
		return pattern.replace(TOPIC_TOKEN, topicPath).replace("/", "_");
	}
}
