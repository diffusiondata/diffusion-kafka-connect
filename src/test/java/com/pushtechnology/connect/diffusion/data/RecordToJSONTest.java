...
public class RecordToJSONTest {
	...
	public void assertOnlyWithSchema(Schema schema, Object value, String json) {
		assertEquals("Can parse with schema", json, jsonFromRecord((sinkRecord().val(schema, value).build())).toJsonString());
	}
	
	public void assertWithAndWithoutSchema(Schema schema, Object value, String json) {
		assertEquals("Can parse with schema", json, jsonFromRecord((sinkRecord().val(schema, value).build())).toJsonString());
		assertEquals("Can parse without schema", json, jsonFromRecord((sinkRecord().val(null, value).build())).toJsonString());
	}
}
