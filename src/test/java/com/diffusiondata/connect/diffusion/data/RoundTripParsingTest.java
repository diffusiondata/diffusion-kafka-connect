...
public class RoundTripParsingTest {
    ...
    public void testNestedArray() throws Exception {
        assertRoundTrip(
            array(type(ARRAY).build()).build(),
            array(array(INT32_SCHEMA)).build(),
    ...
    public void testNestedMap() throws Exception {
        ...
        assertRoundTrip(
            map(STRING_SCHEMA, type(MAP).build()).build(),
            map(STRING_SCHEMA, map(STRING_SCHEMA, INT32_SCHEMA).build()).build(),
            map2);
    }
    ...
    public void testStruct() throws Exception {
        ...
        // Cannot synthesize struct from JSON, closest we can get is
        // Map<String, Object>
        ...
        JSON json2 =
            jsonFromRecord(
                sinkRecord().val(schemaAndValue.schema(),
    ...
    private void assertRoundTrip(Schema toSchema, Schema fromSchema,
        Object value) {
        ...
        JSON json2 =
            jsonFromRecord(
                sinkRecord().val(
                    schemaAndValue.schema(), schemaAndValue.value()).build());
